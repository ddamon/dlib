...
import com.dm.network.JSONUtil;
...
public class NetJSONAdapter extends BeanAdapter<JSONObject> implements INetAdapter {
	...
	private String timelineParam = Consts.netadapter_timeline;
	private String timelineinjson = Consts.netadapter_json_timeline;
		...
		public void doInUI(Response response, Integer transfer) {
			...
			if (list.size() == 0) {
				if (dialoger != null) {
					if (!response.isCache()) {
						if (pageNo > 1) {
							dialoger.showToastShort(mContext, Consts.netadapter_no_more);
						}
					}
				}
				hasMore = false;
			...
			addAll(list);
			// 如果加载的收据不是来自缓存那么就不在使用缓存否则一直使用缓存
			if (!response.isCache) {
