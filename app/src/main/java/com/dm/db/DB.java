...
public class DB {
	...
	public void init(String dbname, int dbversion) {
		this.db = new SqliteDbHelper(Ioc.getApplicationContext(), dbname, dbversion).getWritableDatabase();
	};
	...
	private <T> T cursorToBean(Cursor cursor, Class<T> clazz) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			if (field.getType().equals(Integer.class) || field.getType().equals(int.class)) {
				...
				BeanUtil.setProperty(obj, key, cursor.getInt(cursor.getColumnIndex(column)) == 0 ? false : true);
			}
		}
