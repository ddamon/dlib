...
import com.dm.network.cache.CacheManager;
import com.dm.network.cache.CachePolicy;
import com.dm.network.upload.CancelException;
import com.dm.network.upload.ProgressMultipartEntity;
import com.dm.network.upload.ProgressMultipartEntity.ProgressListener;
...
public class DhNet {
	...
	public void useCache(CachePolicy policy) {
		...
		if (cachePolicy != CachePolicy.POLICY_NOCACHE) {
			if (cacheManager == null) {
				cacheManager = IocContainer.getIocContainer().get(CacheManager.class);
			}
		}
	}
	...
	public DhNet execuse(NetTask task) {
		...
		// 添加全局参数
		globalParams = IocContainer.getIocContainer().get(GlobalParams.class);
		if (globalParams != null) {
			Map<String, String> globalparams = globalParams.getGlobalParams();
			...
			if (cacheManager != null) {
				...
				if (result != null) {
					...
					try {
						...
						DhNet.this.task.doInUI(response, NetTask.TRANSFER_DOUI_ForCache);
						// 缓存有数据就返回
						isCacheOk = true;
			...
			public void run() {
				...
				if (hasNet) {
					...
					try {
						...
						// Log.d("DLib", " method: " + method + DhNet.this.url +
						// " params: " + params + " result: " + result);
						Log.d("DLib", " method: " + method + "\n" + DhNet.this.url + "?" + ps + "\n result: " + result);
						Response response = new Response(result);
						...
						if (code == null) {
							...
							if (cacheManager != null && cachePolicy != CachePolicy.POLICY_NOCACHE) {
								if (response.jo != null) {
									cacheManager.creata(url, params, response.result);
								}
							}
						}
						if (!isCanceled) {
							// 当没有使用缓存或者缓存策略是网后更新
							if (!isCacheOkf || cachePolicy == CachePolicy.POLICY_BEFORE_AND_AFTER_NET) {
		...
		if (cachePolicy == CachePolicy.POLICY_ON_NET_ERROR) {
			if (cacheManager != null) {
				...
				if (result != null) {
					...
					response.isCache(true);
					DhNet.this.task.doInBackground(response);
					DhNet.this.task.transfer(response, NetTask.TRANSFER_DOUI_ForCache);
				}
			}
		}

		String errorjson = "{'success':false,'msg':'网络不给力','code':'noNetError'}";
	...
	private void onNetError(Exception e, Boolean hasUserCache) {
		...
		String errorjson;
		if (isFromCache) {
			errorjson = "{'success':false,'msg':'网络不给力,使用缓存数据','code':'netErrorButCache'}";
		} else {
			errorjson = "{'success':false,'msg':'网络不给力','code':'netError'}";
		}
		Response response = new Response(errorjson);
	...
	public DhNet execuseInDialog(NetTask task) {
		...
		IDialog dialoger = IocContainer.getIocContainer().get(IDialog.class);
		if (dialoger != null) {
