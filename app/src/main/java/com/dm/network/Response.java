...
public class Response {
		...
		if (!TextUtils.isEmpty(result)) {
			...
			if (result.trim().startsWith("{")) {
				try {
					...
					if (jo.has(Consts.response_success)) {
						success = JSONUtil.getBoolean(jo, Consts.response_success);
					}
					if (jo.has(Consts.response_msg)) {
						msg = jo.getString(Consts.response_msg);
					}
					if (jo.has(Consts.response_code)) {
						code = JSONUtil.getString(jo, Consts.response_code);
					}
					if (jo.has(Consts.response_status)) {
						status = JSONUtil.getString(jo, Consts.response_status);
					}
					if (jo.has(Consts.response_data)) {
						data = JSONUtil.getString(jo, Consts.response_data);
					}
					if (jo.has(Consts.response_version)) {
						version = JSONUtil.getString(jo, Consts.response_version);
					}
					if (jo.has(Consts.response_validation)) {
						validationStr = JSONUtil.getString(jo, Consts.response_validation);
						validation = com.alibaba.fastjson.JSONObject.parseObject(validationStr, Validation.class);
					}
	 ...
	 * @return
	 */
	public <T> T result2Model(Class<T> clazz) {
	 ...
	 * @return
	 */
	public <T> List<T> result2List(final Class<T> clazz) {
		...
		List<T> list = JSON.parseObject(result, type);
		return list;
	}
	...
	// return modelFrom(Consts.response_data);
	// }

	public <T> T modelFromData(Class<T> clazz) {
		return modelFrom(clazz, Consts.response_data);
	}
	...
	public <T> List<T> listFromData(Class<T> clazz) {
		return listFrom(clazz, Consts.response_data);
	}
	...
	public boolean isNull(String str) {
		if (str == null || "[]".equals(str) || str.equals("null") || "{}".equals(str)) {
			return true;
		}
		return false;
	}
