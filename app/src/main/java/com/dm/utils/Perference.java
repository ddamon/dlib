...
import com.dm.ioc.anno.FieldsInjectable;
...
public class Perference implements FieldsInjectable, android.os.Parcelable {
	...
	public Perference load() {
		...
		if (!TextUtils.isEmpty(account)) {
			...
			if (infos != null && !"".equals(infos)) {
				Perference perference = JSON.parseObject(infos, getClass());
				if (perference != null) {
	...
	public void clearAll() {
		SharedPreferences p = IocContainer.getIocContainer().getApplicationContext().getSharedPreferences(getClass().getName(), Context.MODE_WORLD_READABLE);
		p.edit().clear().commit();
		notifyDataSetChanged();
	}
